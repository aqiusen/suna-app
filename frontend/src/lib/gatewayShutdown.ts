import { gatewayPath, isNativeDesktopShell } from "./desktopGateway";
...
export async function requestGatewayShutdown(
  ...
  const response = await fetcher(gatewayPath("/api/v1/shutdown"), {
    method: "POST",
  });
  if (!response.ok) {
...
export function isDesktopShell(
  ...
  return new URLSearchParams(search).has("desktop") || isNativeDesktopShell();
}
...
export function bindDesktopUnloadShutdown(
  ...
  const onHide = () => {
    try {
      sendBeacon(gatewayPath("/api/v1/shutdown"));
    } catch {
      // 页面正在卸载，失败也无后续处理。
    }
  };
